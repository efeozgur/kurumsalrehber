...
export default function EditContactPage() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      api.getTitles(),
      api.getContact(id),
    ]).then(([deptRes, titleRes, contactRes]) => {
      setDepartments(deptRes.data);
      setTitles(titleRes.data);
      const c = contactRes.data;
      setForm({
        firstName: c.firstName, lastName: c.lastName, sicilNo: c.sicilNo || '', titleId: c.titleId || 0,
        phoneInternal: c.phoneInternal || '', phoneMobile: c.phoneMobile || '',
        email: c.email || '', departmentId: c.departmentId || 0,
      });
      if (c.avatar) setAvatarPreview(c.avatar);
    }).catch(() => router.push('/admin/contacts'))
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await api.updateContact(id, {
        ...form, titleId: form.titleId || undefined, departmentId: form.departmentId || undefined, avatar: avatarUrl || undefined,
      });
  ...
  return (
    <div className="max-w-3xl mx-auto">
      <div className="flex items-center gap-4 mb-6">
        ...
        <div>
          <h1 className="text-2xl font-bold text-white">Kişi Düzenle</h1>
